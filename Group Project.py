...
def robotMotion(command):
    if("Forward" == command[0]):
        motorPair.move(command[1], unit = "inches")
    elif("Backward" == command[0]):
        motorPair.move(command[1], unit = "inches", speed = -1*motorPair.get_default_speed)
    elif("Spin" == command[0]):
        motorPair.move(command[1], unit = "degrees", steering = 100)
    elif("Arm" == command[0]):
        motorArm.run_for_seconds(seconds = command[1])
    else:
